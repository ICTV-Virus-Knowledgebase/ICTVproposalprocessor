...
<< NEXT >>
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  * RE-DO SQL export using taxonomy_node (not _names)
  * implement re_lineage_tree()
  ...
  for(linenum in linesWoErrors) {
    ...
    if(actionClean %in% c("create") ) {
      ...
      if(is.na(destParentName)) {
        ...
        parentDestNewMatches=(newMSL$taxnode_id==newMSL$tree_id)
      } else {
...
for( code in rownames(proposals)) {
  ...
  # c0de='2022.001A' # create Itzamnaviridae in [tree root]
