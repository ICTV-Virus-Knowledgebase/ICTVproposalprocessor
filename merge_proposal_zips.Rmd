...
<< NEXT >>
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  * process change: not actually added to global newMSL!!!!! Why!?!?
<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
This script 
...
qc_proposal = function(code, proposalDf) {
  ...
  if(params$verbose && length(qc.spaces.leading)>0) { cat("WARNING: ",code," has ",qc.spaces.leading," cells with leading spaces","\n")}
  qc.spaces.trailing =grep(changeDf,pattern=" $")
  if(params$verbose && length(qc.spaces.trailing)>0) { cat("WARNING: ",code," has ",qc.spaces.leading," cells with leading spaces","\n")}
  qc.spaces.embed    =grep(changeDf,pattern=" ") # except species
  if(params$verbose && length(qc.spaces.embed)>0) { cat("WARNING: ",code," has ",qc.spaces.leading," cells with leading spaces","\n")}
  qc.quotes          =grep(changeDf,pattern='"')
  if(params$verbose && length(qc.quotes)>0) { cat("WARNING: ",code," has ",qc.spaces.leading," cells with leading spaces","\n")}
