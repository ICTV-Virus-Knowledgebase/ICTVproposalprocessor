...
<< NEXT >>
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  * report & fix Alphanumeric characters in taxon names
    * allow spaces in species and isolate
...
qc_proposal = function(code, proposalDf) {
  ...
  qc.spaces.leading =grep(changeDf,pattern="^ ")
  if(params$verbose && length(qc.spaces.leading)>0) { 
    cat("WARNING: ",code," has ",length(qc.spaces.leading)," cells with leading spaces","\n");
  }
  qc.spaces.trailing =grep(changeDf,pattern=" $")
  if(params$verbose && length(qc.spaces.trailing)>0) { cat("WARNING: ",code," has ",length(qc.spaces.leading)," cells with leading spaces","\n")}
  qc.spaces.embed    =grep(changeDf,pattern=" ") # except species
  if(params$verbose && length(qc.spaces.embed)>0) { cat("WARNING: ",code," has ",length(qc.spaces.leading)," cells with leading spaces","\n")}
  qc.quotes          =grep(changeDf,pattern='"')
  if(params$verbose && length(qc.quotes)>0) { cat("WARNING: ",code," has ",length(qc.spaces.leading)," cells with leading spaces","\n")}
  
  #TODO: reject embedded spaces, except for species
  #TODO other funny characters (curvy quotes, curvy appostrophies)
  #TODO: diacritical marsk in taxa. 
...
apply_changes = function(code,proposalBasename,changeDf) {
  ...
  for(linenum in rownames(changeDf)) {
    ...
    if(actionClean %in% c("create") ) {
      ...
      #
      curMatches=curMSL$name==as.character(destTaxonName)
      oldMatches=oldMSLs$name==as.character(destTaxonName)
      newMatches=newMSL$name==as.character(destTaxonName)
      # check if exists in the current MSL
      if(sum(curMatches)>0) {
        matchDf = curMSL[curMatches,c("msl_release_num","lineage")]
        ...
        errorDf=addError(errorDf,code,linenum, "ERROR", "CREATE.IN_CUR", "Change=NEW, but 'proposed taxonomy' already exists", 
                         paste0("XLSX ROW ",linenum,
      ...
      if(sum(oldMatches)>0) {
        matchDf = oldMSLs[oldMatches,c("msl_release_num","lineage")]
        matchDf = oldMSLs[order(matchDf$msl_release_num,decreasing = T)[1],]
        matchLineage = paste0( "MSL", matchDf$msl_release_num,":",matchDf$lineage)
        errorDf=addError(errorDf,code,linenum, "ERROR", "CREATE.IN_OLD", "Change=NEW, but 'proposed taxonomy' existed historically", 
                         paste0("XLSX ROW ",linenum,
                                ", proposedTaxonomy=", destTaxonName, 
                                "; existingTaxon=", matchLineage)
        )
      ...
      if(sum(newMatches)>0) {
        matchDf = newMSL[newMatches,c("msl_release_num","lineage")]
        matchDf = newMSL[order(matchDf$msl_release_num,decreasing = T)[1],]
        matchLineage = paste0( "MSL", matchDf$msl_release_num,":",matchDf$lineage)
        errorDf=addError(errorDf,code,linenum, "ERROR", "CREATE.IN_NEW", "Change=NEW, but 'proposed taxonomy' already created in new MSL", 
                         paste0("XLSX ROW ",linenum,
                                ", proposedTaxonomy=", destTaxonName, 
                                "; existingTaxon=", matchLineage,
      ...
      #
      newMatches=newMSL$name==as.character(destParentName)
      if(params$verbose) {print(paste0("CREATE: findParent(",destParentName,")=",sum(newMatches)))}
      ...
      if(sum(newMatches)==0) {
        ...
        newTaxon[1,"in_filename"] = paste0(proposalBasename,".zip")
        ...
        # TODO: genome compositition
        # TODO: host/source
        ## for species only
        # TODO: acccession
        # TODO: exemplar
        # TODO: abbrev
        # TODO: exemplar designation
        # TODO: genome coverage
